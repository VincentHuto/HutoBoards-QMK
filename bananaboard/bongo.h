...
static void draw_bongo(bool minimal)
{
    ...
    oled_set_cursor(0, 2);
    char lyrst[42];
    sprintf(lyrst, "LYR:%03d",(int)Log2(layer_state) );
    oled_write(lyrst,false);
     oled_set_cursor(0, 3);
    char lyrnm[10];
    sprintf(lyrnm, 
    (int)Log2(layer_state)== 0 ? "DEFAULT" :
    (int)Log2(layer_state)== 1 ? "NUMBER1": 
    (int)Log2(layer_state)== 2 ? "NUMBER2" : 
    (int)Log2(layer_state)== 3 ? "FUNCTION": 
    (int)Log2(layer_state)== 4 ? "SPECIAL" : 
    "UNKNOWN" );
